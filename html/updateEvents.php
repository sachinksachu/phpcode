...
if($_SERVER['REQUEST_METHOD']=='POST'){
	...
	if(mysqli_query($con,"UPDATE `event` SET `eventname` = '$eventname',`description` = '$description',`location` = '$location',`event_date` = '$event_date', `event_time` = '$event_time', `photo` = '$img', `seat` = '$seat',`category` = '$category' WHERE `event`.`event_id` = '$event_id' "))
...
//filling response array with values
	...
	//notification
		$sql="select token from people INNER JOIN Booking ON people.user_id = Booking.user_id where Booking.event_id = '$event_id'";
	$result=mysqli_query($con,$sql);
		...
		while($row = mysqli_fetch_assoc($result)) 
		{
			...
			sendFCM("[$eventname','$description','$location','$event_date','$event_time','$img']","UPDATION IN EVENT YOU BOOKED",$row["token"]);
			echo $row["token"];
			echo $eventname;
    		}
